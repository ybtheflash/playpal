...
$qry = sprintf("SELECT password FROM accounts WHERE username = '%s'", mysqli_real_escape_string($dconn, $_SESSION["username"]));
$email = $mob = $name = "";
...
if (isset($_POST['modify'])) {
    ...
    if (empty($opass) || empty($npass) || empty($cnpass)) {
        header('Location: accounts.php');
        exit();
    }

    if (trim($_POST['newpass']) != trim($_POST['cnewpass'])) {
        echo '<script>alert("Confirmation Password Mismatch!");</script>';
    } elseif (password_verify($opass, $dopass) != true) {
        echo '<script>alert("Old Password Wrong!");</script>';
    } else {
        $usr = $_SESSION['username'];
        $pass = password_hash($npass, PASSWORD_DEFAULT);
        $select = mysqli_query($dconn, "UPDATE accounts set password='$pass' where username='$usr'");
        ...
        <div class="w-full max-w-lg p-8">
            ...
            <form action="" method="post">
                <div class="mb-4">
                    ...
                    <input type="text" value="<?php echo $_SESSION["username"]; ?>" id="username" class="w-full p-2 border rounded-md" placeholder="Enter your username" disabled>
                </div>
                <div class="mb-4">
                    <label for="firstName" class="text-sm block font-medium">Current Password</label>
                    <input type="password" id="firstName" name="oldpass" class="w-full p-2 border rounded-md text-black" placeholder="Current Password Here" required>
                </div>
                <div class="mb-4">
                    <label for="firstName" class="text-sm block font-medium">New Password</label>
                    <input type="password" id="firstName" name="newpass" class="w-full p-2 border rounded-md text-black" placeholder="Your New Password" required>
                </div>
                <div class="mb-4">
                    <label for="firstName" class="text-sm block font-medium">Confirm New Password</label>
                    <input type="password" id="firstName" name="cnewpass" class="w-full p-2 border rounded-md text-black" placeholder="Confirm New Password" required>
                </div>
                <button type="submit" name="modify" class="bg-indigo-600 text-white text-sm font-semibold py-2 px-4 rounded-md hover:bg-indigo-700 transition duration-200">Modify Password</button>
            </form>
        </div>
    </div>
