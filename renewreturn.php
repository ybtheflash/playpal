...
<body class="text-white">
    ...
    <div id="Main" class="xl:rounded-r transform  xl:translate-x-0  ease-in-out transition duration-500 flex justify-start items-start h-full  w-full sm:w-64 flex-col">
        <div class="flex flex-col justify-start items-center px-6 w-full  ">
            ...
            <div id="menu1" class="flex justify-start  flex-col w-full md:w-auto items-start pb-1 ">
                ...
                <?php
                if ($userType == 0) {
                    echo '<a href="renewreturn.php"><button class="flex justify-start items-center space-x-6 hover:text-white focus:bg-gray-700 focus:text-white hover:bg-gray-700 text-gray-400 rounded px-3 py-2  w-full md:w-52">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="fixed top-0 left-0 flex items-center justify-center h-full w-full bg-blur" style="z-index: 50;">
        <div class="w-full max-w-lg p-8">
            <h2 class="text-2xl font-semibold mb-4 text-center">Renew | Return Desk</h2>
            <form action="" method="post">
                <?php
                $sql = "SELECT * FROM issues INNER JOIN pldiscs ON pldiscs.d_id = issues.disc_id AND $s_id = issues.issued_by;";
            ...
            <tr>
                ...
                <th>Genre</th>
                <th>Issued On</th>
                <th style="padding-right:10px;">Return</th>
                <th style="padding-right:10px;">Renew</th>
            </tr>';
                    while ($row = $result->fetch_assoc()) {
                        $renewflag = 0;
                        $issued = $row['issued_on'];
                        $sevenDays = date("Y-m-d H:i:s", strtotime($issued . " + 7 days"));
                        $currentDateTime = date("Y-m-d H:i:s");

                        if (strtotime($currentDateTime) >= strtotime($sevenDays)) {
                            $renewflag = 1;
                        }
                        echo '<tr>
                <td style="padding-right:20px;">' . $row['disc_id'] . '</td>
                <td style="padding-right:20px;">' . $row['dname'] . '</td>
                <td style="padding-right:30px;">' . $row['dauthor'] . '</td>
                <td style="padding-right:30px;">' . $row['dgenre'] . '</td>
                <td style="padding-right:30px;">' . $row['issued_on'] . '</td>
                <td style="padding-right:30px;"><a href = "returndisc.php?disc_id=' . $row['disc_id'] . '"><button class="bg-blue-600 text-white text-sm font-semibold py-2 px-4 rounded-md hover:bg-indigo-700 transition duration-200" type="button">Return</button></a></td>
                ';
                        if ($renewflag == 1) {
                            echo '<td style="padding-right:20px;"> <a href = "renewdisc.php?disc_id=' . $row['disc_id'] . '"><button class="bg-yellow-600 text-white text-sm font-semibold py-2 px-4 rounded-md hover:bg-indigo-700 transition duration-200" type="button">Renew</button></a></td>';
                        } else {
                            echo '<td style="padding-right:20px;"><button class="bg-gray-800 text-white text-sm font-semibold py-2 px-4 rounded-md transition duration-200" disabled>Renew</button>
                            </td>';
                        }
                        '</tr>';
                    }
                    ...
                    echo '<h2 class="text-base font-semibold mb-4 text-center">No Discs Issued.</h2>';
                }
    ...
    <!-- <button type="submit" name="update" class="bg-indigo-600 text-white text-sm font-semibold py-2 px-4 rounded-md hover:bg-indigo-700 transition duration-200">Update Details</button> -->
